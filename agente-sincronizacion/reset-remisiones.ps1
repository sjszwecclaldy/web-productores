# Reset del dominio remisiones (remitos de leche) + opcional sync.
# Usa el .env del agente (BACKEND_URL + INGEST_API_KEY).
#
# Uso:
#   # Vaciar toda la tabla y recargar histórico
#   powershell -ExecutionPolicy Bypass -File .\reset-remisiones.ps1
#
#   # Borrar solo desde una fecha (evita timeout SAP) y recargar ese tramo
#   powershell -ExecutionPolicy Bypass -File .\reset-remisiones.ps1 -From 2026-01-01
#
#   # Solo reset, sin correr sync.ps1
#   powershell -ExecutionPolicy Bypass -File .\reset-remisiones.ps1 -NoSync
...
$LogFile = Join-Path $ScriptDir 'reset-remisiones.log'

. (Join-Path $ScriptDir 'sap-common.ps1')

Write-Log '=== Reset remisiones ==='
...
if ($NoSync) {
    ...
    Write-Log '=== Fin reset remisiones ==='
    exit 0
}

Write-Log 'Ejecutando sync.ps1 para recargar REMISION (y demas dominios)...'
